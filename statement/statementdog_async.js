...
Stock.prototype.getStockData = function() {
  return `公司資料: ${this.info}\t 本益比: ${this.pe}\t 股價淨值比:${this.pb}\t 殖利率:${this.cashYield}`;
}


function requestGet(stock, callback) {
  request({
...
async.map(stocks, requestGet, function(error, data) {
  if (error) {
    console.log(error);
  }
  data.forEach(function(data) {
    console.log(data.getStockData());
  });
});
